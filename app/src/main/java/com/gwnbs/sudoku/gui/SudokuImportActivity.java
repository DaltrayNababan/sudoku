...
import com.gwnbs.sudoku.gui.importing.AbstractImportTask;
import com.gwnbs.sudoku.gui.importing.OpenSudokuImportTask;
import com.gwnbs.sudoku.gui.importing.SdmImportTask;
...
public class SudokuImportActivity extends ThemedActivity {
    ...
    private final AbstractImportTask.OnImportFinishedListener mOnImportFinishedListener = (importSuccessful, folderId) -> {
        if (importSuccessful) {
            Intent i;
            if (folderId == -1) {
                // multiple folders were imported, go to folder list
                i = new Intent(SudokuImportActivity.this, FolderListActivity.class);
            } else {
                // one folder was imported, go to this folder
                i = new Intent(SudokuImportActivity.this, SudokuListActivity.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (dataUri != null) {
            ...
            if (dataUri.getScheme().equals("content")) {
                ...
                Log.v(TAG, dataUri.toString());
                try {
                    juri = new java.net.URI(dataUri.getScheme(), dataUri.getSchemeSpecificPart(), dataUri.getFragment());
            ...
            if (cbuf_str.contains("<opensudoku")) {
                ...
                Log.e(TAG, String.format("Unknown type of data provided (mime-type=%s; uri=%s), exiting.",
