...
import com.gwnbs.sudoku.gui.SudokuImportActivity;
import com.gwnbs.sudoku.utils.Const;
import com.gwnbs.sudoku.db.SudokuDatabase;
import com.gwnbs.sudoku.db.SudokuImportParams;
 ...
 *  and {@link #importGame(String)} methods. Note
 * that <code>importFolder</code> must be called first, otherwise <code>importGame</code>
 * doesn't know where to put puzzles.
 * 3) Add code to {@link SudokuImportActivity} which creates instance of your new class and
 * passes it input parameters.
 * <p/>
 * TODO: add cancel support & replace deprecation
 *
 * @author romario
 */
...
public abstract class AbstractImportTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    int mGameCount = 0; //count of processed puzzles
    private String mImportError;
    private boolean mImportSuccessful;
    private final SudokuImportParams mImportParams = new SudokuImportParams();
     ...
     *
     * @param pars Fields to import (state of game, created, etc.)
     */
