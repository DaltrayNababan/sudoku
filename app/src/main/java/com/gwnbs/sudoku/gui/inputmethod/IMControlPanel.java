...
import com.gwnbs.sudoku.gui.HintsQueue;
import com.gwnbs.sudoku.gui.SudokuBoardView;
import com.gwnbs.sudoku.gui.SudokuBoardView.OnCellSelectedListener;
import com.gwnbs.sudoku.gui.SudokuBoardView.OnCellTappedListener;
...
public class IMControlPanel extends LinearLayout {
    ...
    private final List<InputMethod> mInputMethods = new ArrayList<>();
    private int mActiveMethodIndex = -1;
    private final OnCellTappedListener mOnCellTapListener = cell -> {
        if (mActiveMethodIndex != -1) {
            mInputMethods.get(mActiveMethodIndex).onCellTapped(cell);
        }
    };
    private final OnCellSelectedListener mOnCellSelected = cell -> {
        if (mActiveMethodIndex != -1) {
            mInputMethods.get(mActiveMethodIndex).onCellSelected(cell);
        }
    };
    private final OnClickListener mSwitchModeListener = v -> {
        Sound.soundClick(getContext());
    ...
    private void createInputMethods() {
        if (mInputMethods.size() == 0) {
            ...
            addInputMethod(INPUT_METHOD_SINGLE_NUMBER, new IMSingleNumber(getContext()));
            addInputMethod(INPUT_METHOD_NUMPAD, new IMNumpad(getContext()));
        }
    }
