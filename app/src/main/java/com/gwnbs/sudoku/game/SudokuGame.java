...
import com.gwnbs.sudoku.game.command.AbstractCommand;
import com.gwnbs.sudoku.game.command.ClearAllNotesCommand;
import com.gwnbs.sudoku.game.command.CommandStack;
import com.gwnbs.sudoku.game.command.EditCellNoteCommand;
import com.gwnbs.sudoku.game.command.FillInNotesCommand;
import com.gwnbs.sudoku.game.command.FillInNotesWithAllValuesCommand;
import com.gwnbs.sudoku.game.command.SetCellValueAndRemoveNotesCommand;
import com.gwnbs.sudoku.game.command.SetCellValueCommand;
...
public class SudokuGame {
     ...
     */
    public void solveCell(Cell cell, Context context, TextView textHint) {
        mSolver = new SudokuSolver();
        mSolver.setPuzzle(mCells);
        ArrayList<int[]> finalValues = mSolver.solve();
        ...
        for (int[] rowColVal : finalValues) {
            if (rowColVal[0] == row && rowColVal[1] == col) {
                ...
                if (cell.getValue() == val) {
                    ...
                    SudokuPlayActivity.toastM(context, context.getString(R.string.already_filled_correct));
                    return;
                }
                this.setCellValue(cell, val);
                Sound.soundFilled(context);
                SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
                prefs.edit().putInt(Val.HINT, prefs.getInt(Val.HINT, 10) - 1).apply();
                textHint.setText(String.valueOf(prefs.getInt(Val.HINT, 10)));
            }
        }
    }
