...
import com.gwnbs.sudoku.gui.SudokuListFilter;
import com.gwnbs.sudoku.gui.SudokuListSorter;
...
public class SudokuDatabase {
    ...
    public SudokuGame getSudoku(long sudokuID) {
        ...
        db = mOpenHelper.getReadableDatabase();
        try (Cursor c = qb.query(db, null, null, null, null, null, null)) {
            if (c.moveToFirst()) {
    ...
    public List<Integer> getAllSudokuIDs() {
        SQLiteDatabase db = mOpenHelper.getReadableDatabase();
        String selectQuery = "SELECT  * FROM " + SudokuDatabase.SUDOKU_TABLE_NAME;
        Cursor cursor = db.rawQuery(selectQuery, null);
        List<Integer> sudokuIDs = new ArrayList<>();
        if (cursor.moveToFirst()) {
            do {
                sudokuIDs.add(cursor.getInt(cursor.getColumnIndex(SudokuColumns._ID)));
